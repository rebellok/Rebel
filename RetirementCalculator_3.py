...
roth_conversion_start_year = st.sidebar.number_input("Start Year for Roth Conversions",
                                                    min_value=current_year,
                                                    value=current_year)
roth_conversion_end_year = st.sidebar.number_input(
    "End Year for Roth Conversions",
    min_value=roth_conversion_start_year,
    value=max(roth_conversion_start_year, retirement_start_year)
)
...
def apply_tax_deduction(total_deduction, balances):
    """
    Deducts from salary, then brokerage, then traditional, then Roth.
    Does not allow any account to go negative.
    Args:
        total_deduction (float): The total deduction amount to apply.
        balances (dict): Dictionary with keys 'salary', 'brokerage', 'traditional', 'roth'.
    Returns:
        dict: Updated balances after deduction.
    """
    deduction_left = total_deduction
    order = ['salary', 'brokerage', 'traditional', 'roth']
    updated = balances.copy()
    for key in order:
        if deduction_left <= 0:
            break
        available = updated.get(key, 0.0)
        to_deduct = min(available, deduction_left)
        updated[key] = available - to_deduct
        deduction_left -= to_deduct
    return updated
...
def calculate_retirement_projections():
    ...
    for i in range(1, num_years):
        ...
        if enable_salary and salary_start_year <= year <= salary_end_year:
            ...
            # Deduct salary tax from salary first, then allocate to accounts
            salary_after_tax = current_salary - salary_tax
            if salary_after_tax < 0:
                salary_after_tax = 0  # Prevent negative


            # Calculate contributions based on gross salary (before tax)
            ...
            # If salary after tax is less than total contributions, scale down contributions proportionally
            total_contribution = trad_contribution + roth_contribution + brokerage_contribution
            if salary_after_tax < total_contribution and total_contribution > 0:
                scale = salary_after_tax / total_contribution
                trad_contribution *= scale
                roth_contribution *= scale
                brokerage_contribution *= scale
